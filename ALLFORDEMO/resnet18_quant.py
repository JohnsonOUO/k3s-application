...
parser.add_argument(
    '--data_dir',
    default="/workspace/src/Vitis-AI-Quantizer/vai_q_pytorch/example/imagenet",
    help='Data set directory, when quant_mode=calib, it is for calibration, while quant_mode=test it is for evaluation')
parser.add_argument(
    '--model_dir',
    default="/workspace/src/Vitis-AI-Quantizer/vai_q_pytorch/example",
...
#      mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
      mean=[0.4950, 0.4673, 0.4342], std=[0.2991, 0.2942, 0.3053])
  ...
  if train:
    ...
    dataset.targets[dataset.targets == 0] = 418
    dataset.targets[dataset.targets == 1] = 508
    dataset.targets[dataset.targets == 2] = 559
    dataset.targets[dataset.targets == 3] = 620
    dataset.targets[dataset.targets == 4] = 636
    dataset.targets[dataset.targets == 5] = 728
    dataset.targets[dataset.targets == 6] = 738
    dataset.targets[dataset.targets == 7] = 770
    dataset.targets[dataset.targets == 8] = 898
    dataset.samples = [(d, 418) if s == 0
                        else (d, 508) if s == 1 
                        else (d, 559) if s == 2 
                        else (d, 620) if s == 3 
                        else (d, 636) if s == 4 
                        ...
                        else (d, s) for d, s in dataset.samples]
...
#  file_path = os.path.join(args.model_dir, model_name + '.pth.tar')
